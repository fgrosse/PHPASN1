...
class OctetString extends Object implements Parsable
{
    ...
    {
        return safeStrlen($this->value) / 2;
    }
    ...
    protected function getEncodedValue()
    {
        ...
        //Actual content
        while (safeStrlen($value) >= 2) {
            // get the hex value byte by byte from the string and and add it to binary result
            $result .= chr(hexdec(safeSubstr($value, 0, 2)));
            $value = safeSubstr($value, 2);
        }
    ...
    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        ...
        $value = safeSubstr($binaryData, $offsetIndex, $contentLength);
